
  <div class="list-group">
    <a href="/nlc-project1" class="list-group-item list-group-item-action active" aria-current="true">
     Home
    </a>
    <a href="/nlc-project1/nlclogin" class=" list-group-item-action active" aria-current="true">
        Danh sách quản trị
       </a>
       <a href="/nlc-project1/nlc-loaisanpham" class=" list-group-item-action active" aria-current="true">
        Loại sản phẩm
       </a>
       <a href="/nlc-project1/nlc-sanpham" class=" list-group-item-action active" aria-current="true">
        Sản phẩm
       </a>
       <a href="/nlc-project1/nlckhachhang" class=" list-group-item-action active" aria-current="true">
        Khách hàng
       </a>
       <a href="/nlc-project1/nlchoadon" class=" list-group-item-action active" aria-current="true">
        Hóa đơn
       </a>
    
    
  </div>